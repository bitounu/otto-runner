...
{
    uint8_t rA = image[pixA*kBytesPerPixel];
    uint8_t gA = image[pixA*kBytesPerPixel+1];
    uint8_t bA = image[pixA*kBytesPerPixel+2];
    
    uint8_t rB = image[pixB*kBytesPerPixel];
    uint8_t gB = image[pixB*kBytesPerPixel+1];
    uint8_t bB = image[pixB*kBytesPerPixel+2];
    
    image[pixA*kBytesPerPixel] = rB;
    image[pixA*kBytesPerPixel+1] = gB;
    image[pixA*kBytesPerPixel+2] = bB;
    
    image[pixB*kBytesPerPixel] = rA;
    image[pixB*kBytesPerPixel+1] = gA;
    image[pixB*kBytesPerPixel+2] = bA;
}

// just the partition operation from quicksort
int GifPartition(uint8_t* image, const int left, const int right, const int elt, int pivotIndex)
{
    const int pivotValue = image[(pivotIndex)*kBytesPerPixel+elt];
    ...
    {
        int arrayVal = image[ii*kBytesPerPixel+elt];
...
void GifSplitPalette(uint8_t* image, int numPixels, int firstElt, int lastElt, int splitElt, int splitDist, int treeNode, uint32_t buildForDither, GifPalette* pal)
{
    ...
    if(lastElt == firstElt+1)
    {
        if(buildForDither)
        {
            ...
            if( firstElt == 1 )
            {
                ...
                {
                    r = GifIMin(r, image[ii*kBytesPerPixel+0]);
                    g = GifIMin(g, image[ii*kBytesPerPixel+1]);
                    b = GifIMin(b, image[ii*kBytesPerPixel+2]);
                }
            ...
            if( firstElt == (1 << pal->bitDepth)-1 )
            {
                ...
                for(ii=0; ii<numPixels; ++ii)
                {
                    r = GifIMax(r, image[ii*kBytesPerPixel+0]);
                    g = GifIMax(g, image[ii*kBytesPerPixel+1]);
                    b = GifIMax(b, image[ii*kBytesPerPixel+2]);
                }
        ...
        for(ii=0; ii<numPixels; ++ii)
        {
            r += image[ii*kBytesPerPixel+0];
            g += image[ii*kBytesPerPixel+1];
            b += image[ii*kBytesPerPixel+2];
        }
    ...
    for(ii=0; ii<numPixels; ++ii)
    {
        int r = image[ii*kBytesPerPixel+0];
        int g = image[ii*kBytesPerPixel+1];
        int b = image[ii*kBytesPerPixel+2];
    ...
    pal->treeSplit[treeNode] = image[subPixelsA*kBytesPerPixel+splitCom];
    
    GifSplitPalette(image,              subPixelsA, firstElt, splitElt, splitElt-splitDist, splitDist/2, treeNode*2,   buildForDither, pal);
    GifSplitPalette(image+subPixelsA*kBytesPerPixel, subPixelsB, splitElt, lastElt,  splitElt+splitDist, splitDist/2, treeNode*2+1, buildForDither, pal);
}
...
int GifPickChangedPixels( const uint8_t* lastFrame, uint8_t* frame, int numPixels )
{
    ...
    for (ii=0; ii<numPixels; ++ii)
    {
           ...
           lastFrame[2] != frame[2])
        {
            ...
            writeIter[2] = frame[2];
            ++numChanged;
            writeIter += kBytesPerPixel;
        }
        lastFrame += kBytesPerPixel;
        frame += kBytesPerPixel;
    }
...
void GifMakePalette( const uint8_t* lastFrame, const uint8_t* nextFrame, uint32_t width, uint32_t height, int bitDepth, uint32_t buildForDither, GifPalette* pPal )
{
    pPal->bitDepth = bitDepth;
    
    // SplitPalette is destructive (it sorts the pixels by color) so
    // we must create a copy of the image for it to destroy
    int imageSize = width*height*kBytesPerPixel*sizeof(uint8_t);
...
void GifDitherImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
    ...
    // to be propagated
    int32_t* quantPixels = (int32_t*)GIF_TEMP_MALLOC(sizeof(int32_t)*numPixels*kBytesPerPixel);
    
    int ii;
    for( ii=0; ii<numPixels*kBytesPerPixel; ++ii )
    {
    ...
    for( yy=0; yy<height; ++yy )
    {
        ...
        {
            int32_t* nextPix = quantPixels + kBytesPerPixel*(yy*width+xx);
            const uint8_t* lastPix = lastFrame? lastFrame + kBytesPerPixel*(yy*width+xx) : NULL;
            ...
            {
                int32_t* pix7 = quantPixels+kBytesPerPixel*quantloc_7;
            ...
            {
                int32_t* pix3 = quantPixels+kBytesPerPixel*quantloc_3;
            ...
            {
                int32_t* pix5 = quantPixels+kBytesPerPixel*quantloc_5;
            ...
            {
                int32_t* pix1 = quantPixels+kBytesPerPixel*quantloc_1;
...
{
    uint32_t numPixels = width*height, ii;
    for( ii=0; ii<numPixels; ++ii )
    {
        ...
        if(lastFrame) lastFrame += kBytesPerPixel;
        outFrame += kBytesPerPixel;
        nextFrame += kBytesPerPixel;
    }
}
...
void GifWriteLzwImage(FILE* f, uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal)
{
    ...
    for( yy=0; yy<height; ++yy)
    {
        ...
        for( xx=0; xx<width; ++xx)
        {
            uint8_t nextValue = image[(yy*width+xx)*kBytesPerPixel+3];
...
uint32_t GifBegin( GifWriter* writer, const char* filename, uint32_t width, uint32_t height, uint32_t delay, int32_t bitDepth, uint32_t dither)
{
#if _MSC_VER >= 1400
	writer->f = 0;
    ...
    // allocate 
    writer->oldImage = (uint8_t*)GIF_MALLOC(width*height*kBytesPerPixel);
...
uint32_t GifWriteFrame( GifWriter* writer, const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay, int bitDepth, uint32_t dither )
{
    if(!writer->f) return 0;
...
uint32_t GifEnd( GifWriter* writer )
{
    if(!writer->f) return 0;
